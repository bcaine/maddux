...
def main(desired_iterations, save_path):
    ...
    sim = Planning(get_hard_environment_v2(), max_iterations=200)
    ...
    current_controller = DiscreteDeepQ(sim.observation_size, sim.num_actions, brain,
                                       optimizer, session, random_action_probability=0.9,
                                       discount_rate=0.9, exploration_period=1000,
                                       max_experience=10000, store_every_nth=4,
                                       train_every_nth=4, summary_writer=journalist)
    ...
    try:
        for game_idx in range(desired_iterations+1):
            ...
            game = Planning(get_hard_environment_v2(), max_iterations=200)
            game_iterations = 0
